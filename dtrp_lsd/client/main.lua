...
AddEventHandler('esx_lsd:hasEnteredMarker', function(zone)
	...
	if zone == 'OpiumSammler' then
		CurrentAction     = zone
		CurrentActionMsg  = _U('objekt_sammeln')
		CurrentActionData = {}
	elseif zone == 'OpiumVerarbeiter' then
		if kodaQTE >= 5 then
			CurrentAction     = zone
			CurrentActionMsg  = _U('objekt_verarbeiten')
			CurrentActionData = {}
		end
	elseif zone == 'OpiumVerkaeufer' then
...
AddEventHandler('esx_lsd:hasExitedMarker', function(zone)
	...
	if zone == 'OpiumSammler' then
		TriggerServerEvent('esx_lsd:stopHarvestKoda')
	elseif zone == 'OpiumVerarbeiter' then
	TriggerServerEvent('esx_lsd:stopTransformKoda')
	elseif zone == 'OpiumVerkaeufer' then
		TriggerServerEvent('esx_lsd:stopSellKoda')
	end
end)
...
AddEventHandler('esx_lsd:ReturnInventory', function(kodaNbr, kodapNbr, jobName, currentZone)
	kodaQTE			= kodaNbr
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
				if CurrentAction == 'OpiumSammler' then
					TriggerServerEvent('esx_lsd:startHarvestKoda')
				elseif CurrentAction == 'OpiumVerarbeiter' then
					TriggerServerEvent('esx_lsd:startTransformKoda')
				elseif CurrentAction == 'OpiumVerkaeufer' then
					TriggerServerEvent('esx_lsd:startSellKoda')
				end
